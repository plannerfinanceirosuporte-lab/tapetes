            ...
            return (
              <div key={order.id} className="border rounded-xl p-5 bg-white shadow flex flex-col gap-2">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-gray-500 text-sm">{new Date(order.created_at).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' })}</span>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  {items.map(item => {
                    ...
                    if (item.image_url && item.image_url.startsWith('http')) {
                      ...
                      imageUrl = 'https://via.placeholder.com/64x64?text=Produto';
                    }
                    ...
                    return (
                      <div key={item.product_id} className="flex items-center gap-3 border rounded p-2 bg-gray-50">
                        <img src={imageUrl} alt={name} className="w-16 h-16 object-cover rounded bg-white" style={{ background: '#fff' }} />
                ...
                <div className="text-xs text-gray-500 mt-1">Pago em: {new Date(order.created_at).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' })}</div>
              </div>
            );
          })}
        </div>
